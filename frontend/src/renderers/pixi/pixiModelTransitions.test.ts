...
import { mapTableModelToPixiView } from './mapTableModelToPixiView';
import { mapTableModelToPhaserView } from '../phaser/mapTableModelToPhaserView';
...
function pixiViewFor(state: GameState) {
  ...
  return mapTableModelToPixiView({ model, width: 640, height: 480 });
}

function phaserViewFor(state: GameState) {
  const boardFlow = resolveGameBoardFlow({ variant: 'sueca', gameState: state });
  const model = buildTableRenderModel({
    gameState: state,
    variant: 'sueca',
...
describe('pixi model transitions (E1 parity)', () => {
  ...
  it('tracks trick 0 → 4 → 0', () => {
    ...
    expect(pixiViewFor(state).trick).toHaveLength(0);
  ...
  it('new round restores 10 local cards and clears trick', () => {
    ...
    expect(pixiViewFor(mid).localHand).toHaveLength(3);
    expect(pixiViewFor(mid).trick).toHaveLength(1);
    expect(pixiViewFor(mid).interactionEnabled).toBe(false);

    const next = baseState({ round: 2, gameScore: { team1: 0, team2: 1 } });
    const view = pixiViewFor(next);
  ...
  it('matches Phaser mapping for hand/trick counts and interaction', () => {
    const state = baseState({
      currentTrick: [card('hearts', 'K', 'hk')],
      waitingForTrickEnd: false
    });
    const pixi = pixiViewFor(state);
    const phaser = phaserViewFor(state);
    expect(pixi.localHand.map((c) => c.card.id)).toEqual(
      phaser.localHand.map((c) => c.card.id)
    );
    expect(pixi.trick.map((t) => t.card.id)).toEqual(phaser.trick.map((t) => t.card.id));
    expect(pixi.interactionEnabled).toBe(phaser.interactionEnabled);
    expect(pixi.opponents.map((o) => o.backPositions.length)).toEqual(
      phaser.opponents.map((o) => o.backPositions.length)
    );
  });
});