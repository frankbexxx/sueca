  ...
  return (
    <div className="screen-play">
      <header className="screen-header">
        <h1 className="screen-title">{t.playSetup.title}</h1>
        <p className="screen-subtitle">{t.playSetup.subtitle}</p>
      </header>

      <div className="play-setup-card dobo-panel">
        ...
        {MULTIPLAYER_ENABLED && (
          <div className="play-setup-section">
            <label className="checkbox-option">
              <input
                type="checkbox"
                checked={setup.multiplayerEnabled}
                onChange={(e) => setup.setMultiplayerEnabled(e.target.checked)}
              />
              <span>{t.startMenu.enableMultiplayer}</span>
            </label>
          </div>
        )}
