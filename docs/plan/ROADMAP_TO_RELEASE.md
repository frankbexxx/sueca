...
| REL-DATA-02 | Real match history | DONE | Real per-match durable history (`sueca-match-history-v1`) · migrate ≤3 legacy finished summaries (no fabricated rows from aggregate stats) · retention `MAX_MATCH_HISTORY=2000` · product-history ≠ replay · played=160 → next real match = 161 + exactly 1 history record · corrupt-envelope quarantine · duplicate protection · export helper · OPPO `adb install -r` smoke PASS (stats+1 · history+1 · History UI · reopen idempotent). Closed on device validation | P0 | YES (retention UX) | DONE |
...
| Finished list | `sueca-finished-games-v1` retains only **3** summaries (`MAX_FINISHED`) for recent UI — career archive is now `sueca-match-history-v1` (`REL-DATA-02`) |
...
**`REL-DATA-02` evidence:** `sueca-match-history-v1` real per-match records · migrate ≤3 legacy finished summaries (provenance `finished-games-v1`) · **never** fabricate history from aggregate stats · retention `MAX_MATCH_HISTORY=2000` (newest kept) · product history ≠ replay · `played=160` → next King = `161` + exactly 1 history row · corrupt-envelope quarantine survives subsequent writes · export helper · History UI recent slice · focused + full frontend tests · **OPPO Reno13 `adb install -r` PASS** (prefs/sessions preserved · one completion → stats+1 + history+1 · History UI · nav/reopen no duplicate) — **DONE**.
